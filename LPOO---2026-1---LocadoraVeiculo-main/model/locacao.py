...
class Locacao:
    ...
    def __init__(self, veiculo: Veiculo, data_inicio: date=datetime.now().date(), data_fim: date=None, estrategia:CalculoLocacaoStrategy = CalculoPadraoStrategy(), id_locacao=None, status=StatusLocacao.RESERVADO):
        self.id = id_locacao
    ...
    def retirar_veiculo(self):
        if self.status != StatusLocacao.RESERVADO:
            raise Exception("Só é possível retirar um veículo reservado.")
        self.status = StatusLocacao.LOCADO

    def devolver_veiculo(self):
        if self.status != StatusLocacao.LOCADO:
            raise Exception("Só é possível devolver um veículo locado.")
        self.status = StatusLocacao.DEVOLVIDO
        self.devolvido = True

    def cancelar_reserva(self):
        if self.status != StatusLocacao.RESERVADO:
            raise Exception("Só é possível cancelar uma reserva.")
        self.status = StatusLocacao.CANCELADO
